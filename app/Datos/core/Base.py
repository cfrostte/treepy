...
class Base(object):
    ...
    def atributos(self, con_clave=False):
        ...
        if con_clave:
            return [a for a in dir(self) if not a.startswith('__') and not callable(getattr(self, a)) and not a == 'tabla']
        return [a for a in dir(self) if not a.startswith('__') and not callable(getattr(self, a)) and not a == 'tabla' and not a == 'clave']
