...
class Exportador(object):
    ...
    def nuevo_archivo(carpeta, extencion, clave):
        ...
        hh = n.hour
        mm = n.minute
        q_1 = 'Informe del ensayo ID {}'.format(clave)
...
class CSV(Exportador):
    ...
    def informe(db, carpeta, clave=None):
        ...
        with open(archivo, 'w') as a:
            ...
            consulta = """SELECT e.nro AS A, r.nro AS B, c.nro AS C, a.clave AS D, a.latitud AS E, a.longitud AS F,
            CASE WHEN (SELECT 1 FROM arboles_faltantes AS a_f JOIN objetos AS o WHERE a_f.id_arboles = a.clave 
            AND o.tipo = 'ArbolFaltante' AND o.id = a_f.clave AND o.eliminado = 0) THEN 'Si' ELSE 'No' END AS G, a.areaCopa AS H 
            FROM arboles AS a JOIN repeticiones AS r JOIN ensayos AS e 
            JOIN clones AS c JOIN parcelas AS p JOIN bloques AS b 
            JOIN objetos AS o_1 JOIN objetos AS o_2 JOIN objetos AS o_3 
            JOIN objetos AS o_4 JOIN objetos AS o_5 JOIN objetos AS o_6
            WHERE a.id_repeticiones = r.clave AND r.id_ensayos = e.clave 
            AND c.clave = p.id_clones AND p.id_bloques = b.clave AND b.id_repeticiones = r.clave
            AND o_1.tipo = 'Arbol' AND o_1.id = a.clave AND o_1.eliminado = 0
            AND o_2.tipo = 'Repeticion' AND o_2.id = r.clave AND o_2.eliminado = 0
            AND o_3.tipo = 'Ensayo' AND o_3.id = e.clave AND o_3.eliminado = 0
            AND o_4.tipo = 'Clon' AND o_4.id = c.clave AND o_4.eliminado = 0
            AND o_5.tipo = 'Parcela' AND o_5.id = p.clave AND o_5.eliminado = 0
            AND o_6.tipo = 'Bloque' AND o_6.id = b.clave AND o_6.eliminado = 0 {}""".format('AND e.clave = ?' if clave else '')
